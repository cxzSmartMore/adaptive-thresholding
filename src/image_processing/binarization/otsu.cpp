...
void Otsu::ProcessImpl(cv::Mat& input, [[maybe_unused]] const Parameter& param) const {
  ...
  auto total_sum = 0.0;
  for (auto idx = 0U; idx < HISTOGRAM_SIZE; ++idx) {
    total_sum += static_cast<double>(idx * histogram[idx]);
  }
  ...
  for (uint64_t idx = 0; idx < HISTOGRAM_SIZE; ++idx) {
    weight[0] += static_cast<double>(histogram[idx]);
    if (weight[0] <= epsilon) {
      continue;
    }
    ...
    sum[0] += static_cast<double>(idx * histogram[idx]);
